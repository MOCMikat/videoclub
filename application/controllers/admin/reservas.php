...
class Peliculas extends CI_Controller {
  ...
  function index($pag = 1) {
    ...
    list($res, $npags) = $this->paginador->paginar($this->Reserva,
                                                   $columna,
                                                   $criterio,
                                                   $pag);
    ...
    $this->template->load('template', 'reservas/index', $data);
  }
  ...
  function insertar() {
    ...
    if ($this->input->post('insertar') &&
        ...
        $this->Reserva->insertar($this->input->post());
        $this->session->set_flashdata('mensaje', 'La reserva se ha ' .
                                    'insertado correctamente');
      redirect('admin/reservas/index');
      return;
    }

    $this->template->load('template', 'reservas/insertar');
  }
  ...
  function modificar() {
    ...
    if ($this->input->post('modificar') &&
        $this->reglas_validacion() == TRUE) {
        $this->Reserva->modificar($this->input->post());
      $this->session->set_flashdata('mensaje', 'La reserva se ha ' .
    ...
    $this->template->load('template', 'admin/reservas/modificar', $data);
  }
